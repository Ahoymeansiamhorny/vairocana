...
export function getTimeState() {
  ...
  if (hours >= 6 && hours <= 10) return `早安 ⛅`
  if (hours >= 10 && hours <= 14) return `午安 🌞`
  if (hours >= 14 && hours <= 18) return `午安 🌞`
  if (hours >= 18 && hours <= 24) return `晚安 🌛`
  if (hours >= 0 && hours <= 6) return `晚安 🌛`
}
...
export function getFlatMenuList(menuList) {
  ...
  let newMenuList = getShowMenuList(menuList)
...
export function getShowMenuList(menuList) {
  ...
  return newMenuList.filter((item) => {
    if (item.children?.length) {
      ...
      return !item.meta.isHide && item.meta?.permission
    }
  })
}
