<template>
  <div class="table-box">
    <div class="main-box">
          ...
          <el-form label-position="left" label-width="auto" @submit="onSubmit" @reset="resetForm()">
            <el-form-item :label="t('form.fields.ownerId')" v-bind="ownerIdProps" class="mb50">
              <el-select v-model="ownerId">
                <el-option
                  v-for="item in proTable?.enumMap.get('ownerId')"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
            ...
            <el-form-item :label="t('form.fields.display')" v-bind="displayProps" class="mb50">
              <el-select v-model="display">
                <el-option
                  v-for="item in proTable?.enumMap.get('display')"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
      ...
      <div class="table-box" v-loading="loadingT">
        <ProTable
          ref="proTable"
          :data="filterData"
          :columns="columns"
...
import { numberCategory, numberDisplay, numberStatus } from '@/utils/dict'
...
const rule = zod.object({
  ...
  description: zod.union([
    ZOD_LITERAL('', ONE_OF),
    ZOD_LITERAL('OT', ONE_OF),
    ZOD_LITERAL('NEW SIM', ONE_OF)
  ])
})
  ...
  await new Promise((resolve) => {
    setTimeout(() => {
      for (const i of tableData.value) {
        const k = (`${i[number.value]}` || '').replace('-', '').replace(/\s+/g, '')
        const v = {
          id: i['__rowNum__'],
          number: k,
...
const number = ref('Phone')
const price = ref('Market Price')
const description = ref('Remark')
