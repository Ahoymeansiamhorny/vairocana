<script setup>
import { ref } from 'vue'
import { PurchaseRefundRequest } from '@/api/purchase/purchase'
...
const RefundOrder = (row) => {
  PurchaseRefundRequest({ order_id: row.order_id }).then(() => {
    const message = `${t(`form.action.refund`, {
...
const title = t('form.titles.refund') + ' - ' + dialogProps.value.row.purchase.order_id
</script>
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button
          ...
          type="primary"
          @click="RefundOrder(dialogProps.row.purchase)"
        >
